 ...
 */
public class RedBlackTreeAE<T extends Comparable<T>> extends RedBlackTree<T> implements RedBlackTreeInterfaceAE<T> {
...
//----------------------------------------------------------------------------
    ...
    public T next(T data) {
        Node<T> current = findNodeWithData(data);
        if (current == null) {
        ...
        if (current.context != null) {
            if (current.context[2] != null) {
                ...
                while (current.context[0] != null && current == current.context[0].context[2]) {
    ...
    public T minimum(T data) {
        Node<T> node = findNodeWithData(data);
    ...
    /**
     * method that returns the data of a specific node that is found
     * @param data the data that is beind search in the red black tree
     * @return the data from the node that is found
     */
