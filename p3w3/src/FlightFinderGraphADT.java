 ...
 */
public class FlightFinderGraphADT implements FlightFinderGraphADTInterface {
    protected class Node {
        public AirportInterface data;
        public List<FlightFinderGraphADT.Edge> edgesLeaving = new LinkedList<>();
        public List<FlightFinderGraphADT.Edge> edgesEntering = new LinkedList<>();
        public Node(AirportInterface data) { this.data = data; }
    }
    // Nodes can be retrieved from this hashtable by their unique data
    protected Hashtable<AirportInterface, FlightFinderGraphADT.Node> nodes = new Hashtable();
    ...
    protected class Edge {
        ...
        public FlightFinderGraphADT.Node predecessor;
        public FlightFinderGraphADT.Node successor;
        public Edge(CostInterface data, FlightFinderGraphADT.Node pred, FlightFinderGraphADT.Node succ) {
            this.data = data;
     ...
     */
    protected class SearchNode implements Comparable<FlightFinderGraphADT.SearchNode> {
        public FlightFinderGraphADT.Node node;
        public double cost;
        public FlightFinderGraphADT.SearchNode predecessor;
        public SearchNode(FlightFinderGraphADT.Node node, double cost, FlightFinderGraphADT.SearchNode predecessor) {
            this.node = node;
            this.cost = cost;
    ...
    public boolean insertNode(AirportInterface data) {
        ...
        nodes.put(data, new FlightFinderGraphADT.Node(data));
        return true;
    }
    ...
    public boolean removeNode(AirportInterface data) {
        ...
        FlightFinderGraphADT.Node oldNode = nodes.remove(data);
        // remove all edges entering neighboring nodes from this one
        for(FlightFinderGraphADT.Edge edge : oldNode.edgesLeaving)
            edge.successor.edgesEntering.remove(edge);
        // remove all edges leaving neighboring nodes toward this one
        for(FlightFinderGraphADT.Edge edge : oldNode.edgesEntering)
    ...
    public boolean insertEdge(AirportInterface pred, AirportInterface succ, CostInterface weight) {
        ...
        FlightFinderGraphADT.Node predNode = nodes.get(pred);
        FlightFinderGraphADT.Node succNode = nodes.get(succ);
        if(predNode == null || succNode == null) return false;
        try {
            // when an edge already exists within the graph, update its weight
            FlightFinderGraphADT.Edge existingEdge = getEdgeHelper(pred,succ);
            existingEdge.data = weight;
        } catch(NoSuchElementException e) {
            // otherwise create a new edges
            FlightFinderGraphADT.Edge newEdge = new FlightFinderGraphADT.Edge(weight,predNode,succNode);
            this.edgeCount++;
    ...
    public boolean removeEdge(AirportInterface pred, AirportInterface succ) {
        try {
            // when an edge exists
            FlightFinderGraphADT.Edge oldEdge = getEdgeHelper(pred,succ);
    ...
    protected FlightFinderGraphADT.Edge getEdgeHelper(AirportInterface pred, AirportInterface succ) {
        FlightFinderGraphADT.Node predNode = nodes.get(pred);
        // search for edge through the predecessor's list of leaving edges
        for(FlightFinderGraphADT.Edge edge : predNode.edgesLeaving)
     ...
     */
    protected FlightFinderGraphADT.SearchNode computeShortestPath(AirportInterface start, AirportInterface end) throws NoSuchElementException {
        ...
        double cost = 0;
        FlightFinderGraphADT.SearchNode predecessor;

        Hashtable<AirportInterface, FlightFinderGraphADT.Node> visitedNodes = new Hashtable();
        PriorityQueue<FlightFinderGraphADT.SearchNode> pq = new PriorityQueue<FlightFinderGraphADT.SearchNode>();
        FlightFinderGraphADT.SearchNode startSearchNode = new FlightFinderGraphADT.SearchNode(nodes.get(start), 0, null);
        pq.add(startSearchNode);

        while (!pq.isEmpty()) {
            FlightFinderGraphADT.SearchNode currentNode = pq.remove();
            if (!visitedNodes.containsKey(currentNode.node.data)) {
                ...
                for (FlightFinderGraphADT.Edge edge : currentNode.node.edgesLeaving) {
                    FlightFinderGraphADT.SearchNode unvisitedSuccessor = new FlightFinderGraphADT.SearchNode(edge.successor, cost+edge.data.getCost(), currentNode);
    ...
    public List<AirportInterface> shortestPathData(AirportInterface start, AirportInterface end) {
        ...
        FlightFinderGraphADT.SearchNode endNode = computeShortestPath(start, end);
     ...
     */
    private void addNodeToList(List<AirportInterface> path, FlightFinderGraphADT.SearchNode node) {
        if (node == null) {
            return;
        }
     ...
     */
    private void findAllPathsHelper(AirportInterface  start, AirportInterface end, Double cost, Hashtable<AirportInterface, FlightFinderGraphADT.Node> visitedNodes, List<AirportInterface> localPathList, Hashtable<List<AirportInterface>, Double> allPaths) {
